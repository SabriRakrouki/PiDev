...
public class PostServiceImpl implements IPostService {
	...
	public PostServiceImpl(tn.esprit.repositories.PostRepository postRepository, CommentRepository commentRepository,
			LikesRepository likeRepository, TopicRepository tr, UserRepository userRepo) {
	...
	public PostResponse getAllPostsbyPagination(int pageNo, int pageSize, String sortBy, String sortDir) {
		Sort orderBy = sortDir.equalsIgnoreCase(Sort.Direction.ASC.name()) ? Sort.by(sortBy).ascending()
				: Sort.by(sortBy).descending();
		Pageable pageable = PageRequest.of(pageNo, pageSize, orderBy);
		Page<Post> pages = PostRepository.findAll(pageable);
		List<Post> pagesListEntity = pages.getContent();
		List<PostDto> posts = pagesListEntity.stream().map(pageEntity -> mapToDto(pageEntity))
	...
	/*
	 * @Override public List<Topic> findByTopic(int id_topic) {
	 * 
